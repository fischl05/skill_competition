...
void NEXTION_puts(char *ID, char *string){
	...
	HAL_UART_Transmit(&huart1, buf_str, len, 100);
...
void nextion_instruction_set(char* inst){
	HAL_UART_Transmit(&huart1, (uint8_t*)inst, strlen(inst), HAL_MAX_DELAY);
	HAL_UART_Transmit(&huart1, cmd_end, 3, HAL_MAX_DELAY);
}

coordinate get_touch_coordinate(void){
	uint8_t Rx_Data[9];
	coordinate pos = { 0, 0 };
	HAL_StatusTypeDef res = HAL_UART_Receive(&huart1, Rx_Data, 9, HAL_MAX_DELAY);
	if(res != HAL_OK) return pos;
	if(Rx_Data[0] != 0x67) return pos;
	if(Rx_Data[5] != 0x00) return pos;
