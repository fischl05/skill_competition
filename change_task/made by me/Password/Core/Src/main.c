...
typedef enum{ menu, temp, hum, co2, input_password, password_set }page_name;
...
typedef struct _user_password_control{
	/* struct reset function */
	void (*struct_rest_fuc)(struct _user_password_control* user_password);
	...
	char save_password[5];
	uint8_t input_num;

	/* password input try number */
	uint8_t try_num;
	FIRSTF_STATE try_ok;
	uint8_t try_second;
	uint16_t try_cnt;
	...
	/* user using function */
	void (*touch_sensing_fuc)(void);
	void (*touch_pad_fuc)(void);

	/* input password function */
	void (*input_password_main_fuc)(void);
	void (*input_password_screen_fuc)(void);

	/* password set function */
	void (*password_set_main_fuc)(void);
	void (*password_set_screen_fuc)(void);
}USER_PASSWORD_CONTROL;
...
RESET_STATE reset_state = etc_reset;
...
USER_PASSWORD_CONTROL    password_fuc;
...
uint8_t eeprom_load(uint8_t addr);
void eeprom_load_string(char* string, uint8_t addr);
...
/* eeprom save load function start */
void eeprom_save(uint8_t data, uint8_t addr){
...
void password_input_screen_fuc(void){
	if(firF == run_state){
		...
		eeprom_save(1, INPUT_PASSWORD_ADDR);
	}
...
void password_touch_sensing_fuc(void){
	...
	for(uint8_t i = 0 ; i < 4 ; i++){
		for(uint8_t j = 0 ; j < 3 ; j++){
			uint16_t x_buf = (j + 1) * 50;
			uint16_t y_buf = (i + 1) * 50;
...
void password_input_check_fuc(void){
	if(strcmp(password_fuc.save_password, password_fuc.input_password) == 0){
		...
		Re = 0; firF = run_state;
		eeprom_save(0, INPUT_PASSWORD_ADDR);
		HAL_Delay(1000);
	}
	else {
